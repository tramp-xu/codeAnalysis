...
function init(modules, domApi) {
    ...
    /**cbs = {
     *  create: (6) [ƒ, ƒ, ƒ, ƒ, ƒ, ƒ]
     *  destroy: [ƒ]
     *  remove: [ƒ]
     *  update: (6) [ƒ, ƒ, ƒ, ƒ, ƒ, ƒ]
     * }
     */
    
    //module中的方法是每个vnode转化为dom时必须要执行的，也就是数据转化为对应的样式，类和事件，
    //不同于每个vnode中hook里面自己定义的钩子函数可有可无，
    //vNode中hook里面自己定义的钩子函数是让调用者在某个时机做一些事情用的，类似vue的生命周期钩子
    ...
    //  比较新旧vnode时，只需要向dom中添加、移动或者删除dom即可，新的vnode就是我们需要保存的当前的vnode，
    // 下次他就成了老vnode（第一轮newVnodes就是下一轮的oldVnodes）
    //  比较vnodes是从父节点层层向下，创建dom是从子节点向上

    //  patchVnode做了三件事：1.更新当前节点对应dom的属性（class、listeners、props、style...），
    // 2.调用vnode.data.hook中的update函数。
    // 3.比较新旧vnode的子节点

    // 更新子节点，以及添加子节点的操作 会将其插入 insertedVnodeQueue 队列
    function patchVnode(oldVnode, vnode, insertedVnodeQueue) {
        ...
        2: ƒ updateDataset(oldVnode, vnode)
        3: ƒ updateEventListeners(oldVnode, vnode)
        4: ƒ updateProps(oldVnode, vnode)
        5: ƒ updateStyle(oldVnode, vnode) */
        // cbs.update是一个数组，其中包括更新class、props、listeners、attributes等上方属性的更新方法, 所以这里是遍历更新 class props等
